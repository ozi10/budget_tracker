    ...
    <div style={{ 
      ...
      <form onSubmit={handleSubmit} 
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 20 }}>
          ...
          <div>
            <h1 style={{ fontFamily: "'Fraunces', Georgia, serif", fontSize: 24, color: "#1A202C", margin: 0, fontWeight: 700, letterSpacing: -0.5 }}>Create Account</h1>
            <p style={{ color: "#64748B", fontSize: 13, margin: "2px 0 0" }}>Start your private financial ledger</p>
          </div>
        </div>
        ...
        <button type="submit" disabled={loading}
          style={{ width: "100%", padding: "14px", borderRadius: 16, border: "none", background: "#D97706", color: "#FFFFFF", fontWeight: 700, fontSize: 15, cursor: "pointer", marginTop: 22, display: "flex", alignItems: "center", justifyContent: "center", gap: 8, boxShadow: "0 6px 16px rgba(217,119,6,0.3)", transition: "all .2s" }}>
          {loading ? <><Loader2 size={18} className="animate-spin" /> Creating…</> : <>Get Started <ArrowRight size={16} strokeWidth={2.5} /></>}
        </button>

        <p style={{ textAlign: "center", fontSize: 13, marginTop: 20, color: "#64748B" }}>
          Already have an account? <Link href="/login" style={{ color: "#D97706", fontWeight: 700, textDecoration: "none" }}>Sign in</Link>
        </p>
      </form>
    </div>
  );
